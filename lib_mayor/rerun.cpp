/**
  * rerun.cpp - Super basic rerun system, warning: NOT GENERALIZED ;-)
  */
...
std::string rerunAction;

void rerunReset() 
{
  Brain.resetTimer();
  timer( clear() );
  Brain.TimerClearTimer(T1);
...
void rerunHandle() 
{
  ...
  if (vexRT[Btn7D]) 
  {
    ...
    else if (abs(SensorValue[driveLeft]) > 200 && SensorValue[driveLeft] + SensorValue[driveRight] < 100) 
    { 
      // Turning
      rerunAction = "TURN";
